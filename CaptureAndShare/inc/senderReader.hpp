...
public:
  ...
  void updateCameraList(const std::vector<cameraInfo> &cameras, int id);

  int checkForRequestedCameraId();
  int updateCameraId(int Id);
...
private:
  ...
  int m_camera_list_shm_fd;
  SHM_cameraList *m_camera_list_shm_ptr = nullptr;
