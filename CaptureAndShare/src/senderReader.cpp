...
senderReader::senderReader(ImageInfo img_info): m_image_info(img_info){
    setupBuffer(img_info);
    /*setup shared memory*/
...
int senderReader::setupBuffer(ImageInfo img_info) {
    ...
    const char* shm_path_cam_setup = CAMERA_CONTROLS_SHM_FULL_PATH;

    if (!fileExists(shm_path_cam_setup)) {
        // Shared memory file doesn't exist, create & truncate it
        m_guider_camera_controls_shm_fd = shm_open(CAMERA_CONTROLS_SHM, O_RDWR | O_CREAT, 0666);
        if (m_guider_camera_controls_shm_fd == -1) {
            perror("shm_open create");
            return -1;
        }
        if (ftruncate(m_guider_camera_controls_shm_fd, sizeof(SHM_cameraControls)) == -1) {
            perror("ftruncate");
            close(m_guider_camera_controls_shm_fd);
            return -1;
        }
        printf("Created and truncated shared memory to fixed size\n");
    } else {
        // Exists, open without creating
        m_guider_camera_controls_shm_fd = shm_open(CAMERA_CONTROLS_SHM, O_RDWR, 0666);
        if (m_guider_camera_controls_shm_fd == -1) {
            perror("shm_open");
            return -1;
        }
    ...
    SHM_cameraControls* ptr = static_cast<SHM_cameraControls*>(
        mmap(nullptr, sizeof(SHM_cameraControls),
            PROT_READ | PROT_WRITE, MAP_SHARED, m_guider_camera_controls_shm_fd, 0));
    ...
    m_guider_camera_controls_shm_ptr = ptr;
