...
constexpr ImageBayerPattern getBayerFromSVBBayer(SVB_BAYER_PATTERN pattern)
{
    ...
    case SVB_BAYER_GB:
        return GRBG;
    default:
        return UNKNOWN_PATT; // or throw/handle invalid enum
    }
}
...
constexpr ImageBayerPattern getImageBayerFromASI(ASI_BAYER_PATTERN pattern)
{
    ...
    case ASI_BAYER_GB:
        return GRBG;
    default:
        return UNKNOWN_PATT; // or static_assert(false, "Invalid Bayer pattern");
    }
}
...
constexpr ImageDataType getImageTypeFromSVBImageType(SVB_IMG_TYPE type)
{
    ...
    case SVB_IMG_Y16:
        return Y16;
    default:
        return UNKNOWN_DATA_TYPE; // or throw/handle invalid enum
    }
}
...
constexpr ImageDataType getImageDataTypeFromASI(ASI_IMG_TYPE type)
{
    ...
    case ASI_IMG_Y8:
        return Y8;
    default:
        return UNKNOWN_DATA_TYPE; // or handle error
    }
}
...
constexpr ASI_IMG_TYPE getASIImageTypeFromImageDataType(ImageDataType type)
{
    ...
    default:
        return ASI_IMG_END; // or another invalid/unknown enum value
    }
}
...
constexpr SVB_IMG_TYPE getSVBImageTypeFromImageDataType(ImageDataType type)
{
    ...
    case Y16:
        return SVB_IMG_Y16;
    default:
        return SVB_IMG_END; // or handle invalid case appropriately
    }
}
...
int cameraControl::setupCamera(cameraSetup cam_setup)
{
    ...
    m_timeout_limit = m_ms_per_frame * 2 + 2000;
...
void cameraControl::captureVideoThreadFunc()
{
    ...
    else
    {
        ...
        {
            scanForImage();
            // if (cameraAliveCheck() == false)
            // {
            //     m_camera_opened = false;
            //     m_stop_video_thread = true;
            //     std::cerr << "Camera not responding ..." << std::endl;
            // }
            ...
            timeout_occured = duration_cast<milliseconds>(steady_clock::now() - m_last_send_time).count() >= m_timeout_limit;
...
bool cameraControl::cameraAliveCheck()
{

    if (m_current_camera.producer == "SVB")
    {
        long value = 0;
        SVB_BOOL autoMode = SVB_FALSE;
        auto rc = SVBGetControlValue(m_current_camera.ID, SVB_GAIN, &value, &autoMode);
        if (rc != SVB_SUCCESS)
        {
            std::cerr << "Camera not responding ... " << std::endl;
            return false;
    ...
    else if (m_current_camera.producer == "ZWO")
    {
        long value = 0;
        int num_of_ctrls;
        auto rc = ASIGetNumOfControls(m_current_camera.ID, &num_of_ctrls);
        if (rc != ASI_SUCCESS)
        {
            std::cerr << "Camera not responding ... " << std::endl;
            return false;
...
std::ostream &operator<<(std::ostream &os, const cameraInfo &cam)
{
    ...
    for (size_t i = 0; i < cam.img_data_types.size(); ++i)
    {
        ...
        if (i != cam.img_data_types.size() - 1)
            os << ", ";
    }
    os << "\n";
...
int cameraControl::startVideoCapture()
{
    if (m_camera_opened == false)
    {
        std::cerr << "No camera opened, can't start capture" << std::endl;
        return -1;
    }

    if (m_current_camera.producer == "SVB")
    {
        auto rc = SVBStartVideoCapture(m_current_camera.ID);
        if (rc != SVB_SUCCESS)
        {
            std::cerr << "Failed to start videoCapture for SVB" << std::endl;
            return -1;
        }
        else
        {
    ...
    {
        auto rc = ASIStartVideoCapture(m_current_camera.ID);
        if (rc != ASI_SUCCESS)
        {
            std::cerr << "Failed to start videoCapture for ASI" << std::endl;
            return -1;
        }
        else
        {
...
int cameraControl::stopVideoCapture()
{
    if (m_camera_opened == false)
    {
        std::cerr << "No camera opened, can't stop capture" << std::endl;
        return -1;
    }

    if (m_current_camera.producer == "SVB")
    {
        auto rc = SVBStopVideoCapture(m_current_camera.ID);
        if (rc != SVB_SUCCESS)
        {
            std::cerr << "Failed to stop videoCapture for SVB" << std::endl;
            return -1;
        }
        else
        {
    ...
    {
        auto rc = ASIStopVideoCapture(m_current_camera.ID);
        if (rc != ASI_SUCCESS)
        {
            std::cerr << "Failed to stop videoCapture for ASI" << std::endl;
            return -1;
        }
        else
        {
    ...
    {
        std::cerr << "Unknown camera producer, can't stop video capture!" << std::endl;
        return -1;
    }
}
