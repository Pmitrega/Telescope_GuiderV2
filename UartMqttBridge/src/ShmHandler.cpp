...
ShmHandler::ShmHandler()
{
    ...
    openShm(CAMERA_LIST_SHM, sizeof(SHM_cameraList), reinterpret_cast<void **>(&m_shm_camera_list_ptr));
...
void ShmHandler::readCameraList(SHM_cameraList &camera_list)
{
    camera_list = *m_shm_camera_list_ptr;
}

int ShmHandler::selectCamera(int camera_id)
{
    if (m_shm_camera_list_ptr == nullptr)
    {
        return -1;
    }

    if (camera_id < -1 || camera_id >= MAX_NUMER_OF_CAMERAS)
    {
        return -1;
    }

    if (!m_shm_camera_list_ptr->available[camera_id])
    {
        return -1;
    }

    m_shm_camera_list_ptr->selectedCameraId = camera_id;
    return 0;
}
